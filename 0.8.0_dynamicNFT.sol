...
//   mapping (uint256 => Page) public pages;
  ...
  // public
  function mint(string memory _newPosterName, string memory _newPostText, string memory _newPostImage) public {
    ...
    _safeMint(0x000000000000000000000000000000000000dEaD, supply + 1);

    Post memory newPost = Post("","","");
    Post storage currentPost = posts[supply + 1];

    posts[supply + 1] = newPost;

    currentPost.posterName = _newPosterName;
    currentPost.postText = _newPostText;
    currentPost.postImage = _newPostImage;
...
//   }
  ...
  function fetchPosterName(uint256 _tokenId) public view returns (string memory message){
    Post memory currentPost = posts[_tokenId];
    currentPost.posterName = "Anonymous";
    return (currentPost.posterName);
  }

  function fetchPostText(uint256 _tokenId) public view returns (string memory message){
    Post memory currentPost = posts[_tokenId];
    return (currentPost.postText);
  }

  function fetchPostImage(uint256 _tokenId) public view returns (string memory message){
    Post memory currentPost = posts[_tokenId];
    return (currentPost.postImage);
  }

//   function readMessage(uint256 _tokenId) public view returns (string memory message){
//     Post memory currentPage = posts[_tokenId];
//     return currentPage.postText;
//   }
